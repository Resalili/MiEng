...
struct Quat2D
{
    float cosTheta;
    float sinTheta;
    Quat2D(float angle = 0.0f): cosTheta(std::cosf(angle / 2.0f)), sinTheta(std::sinf(angle / 2.0f)) {}
    Quat2D(float cosTheta, float sinTheta): cosTheta(cosTheta), sinTheta(sinTheta) {}
